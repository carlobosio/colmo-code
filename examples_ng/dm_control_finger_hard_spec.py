...
def objective_function(params: np.ndarray,
    ...
    env = suite.load(domain_name="finger", task_name="turn_hard") 
...
def heuristic(obs: np.ndarray) -> np.ndarray:
    ...
    obs size is 12: position (4), velocity (3), touch (2), target_position (2), dist_to_target (1) return shape is (2,).
    """
