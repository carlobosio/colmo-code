...
def sample_worker(sampler, iter, stop_event: mp.Event):
    """Function to run a single sampler."""
    sampler.initialize_llm()
    try:
        while iter != 0 and not stop_event.is_set():
            should_stop = sampler.sample()
            if should_stop:
                stop_event.set()
            if iter > 0:
                iter -= 1
...
def run_parallel(samplers, database, iterations: int = -1):
    ...
    try:
        ...
        for s in samplers:
            ...
            p = mp.Process(target=sample_worker, args=(s,iterations, stop_event))
