...
class DummySandbox:
  ...
  def run(
    ...
    out = namespace[function_to_run](test_input)
    if isinstance(out, (int, float)):
      return out, True
    elif isinstance(out, tuple) and isinstance(out[0], (int, float)):
      return out, True
    else:
      return None, False
...
# class ProcessPoolSandbox(DummySandbox):
#   """Sandbox that runs DummySandbox functionality in a separate mp process."""

#   def __init__(self, base_path: pathlib.Path, timeout_secs: int = 30, num_processes: int = 4):
#     super().__init__()
...
#     # Create a process pool that we'll reuse
#     self.pool = mp.Pool(processes=num_processes)

#   @staticmethod
#   def _worker_process(program: str, function_name: str, test_input: Any, 
#                       error_file: pathlib.Path):
#     """Worker process that runs the DummySandbox logic."""
#     try:
#       sys.stderr = open(error_file, 'w')
      
#       # This is exactly what DummySandbox.run() does
#       namespace = {}
#       parsed_code = ast.parse(program)
#       compiled_code = compile(parsed_code, filename="<ast>", mode="exec")
#       exec(compiled_code, namespace)
      
#       result = namespace[function_name](test_input)
#       return result, True

#     except Exception as e:
#       logging.debug(f"mp Worker process failed: {e}")
#       return None, False
...
#     try:
#       # Use apply_async to submit task to pool
#       async_result = self.pool.apply_async(
#           self._worker_process, 
#           args=(program, function_to_run, test_input, error_file)
#       )
...
#     except Exception as e:
#       logging.error(f"Could not execute code: {e}")
#     self._save_diagnostics(program, call_data_folder)
#     return None, False
...
if __name__ == "__main__":
   # test the ProcessPoolSandbox
   sandbox = ProcessPoolSandbox(pathlib.Path("./data"))
   program = """
def print_hello(x):
    print('Hello,', x)
    return 42  # Return some value
"""
   result, success = sandbox.run(program, "print_hello", "Carlo", 10)
