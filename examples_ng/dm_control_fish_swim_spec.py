"""Finds a heuristic policy for the fish task. A fish is required to swim to a target.
The fish is rewarded for righting itself with respect to the vertical and also rewarded for swimming to the target.
...
def heuristic(obs: np.ndarray) -> np.ndarray:
    ...
    obs size is 21: joint_angles (7), upright (1), velocity (13). return shape is (5,).
    """
    # action = np.zeros((2,)) + 1
    action = np.random.random(size=5)
    return action